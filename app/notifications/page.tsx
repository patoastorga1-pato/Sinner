...
function notificationCategory(type: string) {
  if (type.includes("booking")) return "Reservation";
    ...
    <AccountShell title="Notifications" copy="Account, booking, message and verification updates.">
      {notifications.length ? (
        <div>
          ...
          <div className="divide-y hairline border-y hairline">{notifications.map((item) => <article key={item.id} className={`py-5 ${item.read_at ? "opacity-65" : ""}`}><div className="flex items-start justify-between gap-5"><div><p className="mb-2 text-xs font-semibold uppercase text-sinner-goldSoft">{notificationCategory(item.type)}</p><h2 className="font-semibold text-sinner-ivory">{item.title}</h2><p className="mt-2 text-sm leading-6 text-sinner-mist">{item.body}</p></div>{!item.read_at ? <span className="mt-2 h-2 w-2 shrink-0 rounded-full bg-sinner-violet" /> : null}</div><time className="mt-3 block text-xs text-sinner-mist/60">{new Date(item.created_at).toLocaleString()}</time></article>)}</div>
        </div>
