...
import { getAdminHostApplications, getHostApplicationCounts, type AdminHostApplication } from "@/lib/data-access/admin";
...
export default async function AdminPage({ searchParams }: { searchParams: Promise<{ error?: string; success?: string }> }) {
  ...
  const [params, applications, counts] = await Promise.all([
    searchParams,
  ...
  return (
    <main>
      ...
      <section className="mx-auto max-w-7xl px-4 py-14 sm:px-6 md:py-20 lg:px-8">
        ...
        <h1 className="mt-3 font-display text-5xl font-medium text-sinner-ivory sm:text-6xl">Security dashboard</h1>
        <p className="mt-4 max-w-2xl leading-7 text-sinner-mist">Phase 1 admin tools for host approval. The full admin panel for users, listings, payments, support and reports remains a later phase.</p>
        <div className="mt-8">
